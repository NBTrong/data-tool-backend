...
const config: { [key: string]: K.Config } = {
  production: {
    ...
    connection: {
      ...
      database: process.env.DB_DATABASE_QUEUE,
    ...
    migrations: {
      ...
      extension: 'ts',
      directory: '../database/migrations/queue',
    },
    seeds: {
      extension: 'ts',
      directory: '../database/seeders/queue',
    },
  },
};
...
export async function checkQueueConnection() {
  try {
    ...
    console.log(
      'Connection has been established successfully.  (queue database)',
    );
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error('Unable to connect to the database:  (queue database)', error);
  }
}
